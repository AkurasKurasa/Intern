# Intern — project guide for Codex
...
- **ALWAYS announce the agent.** Start every reply with `Agent: <name>` (e.g.
  `Agent: Inline`, `Agent: scrollbar-drag-solver`) then a blank line, so it's
  clear who is responding — the main inline assistant or a delegated subagent.
...
- **Commits:** plain messages, **no "Co-Authored-By: Codex" trailer**. Branch
...
The AI assistant is the **guardian of `DEVELOPERS.md`** — it is the single source of
progress truth for this project. This means:
