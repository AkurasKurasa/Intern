...
- [x] `execution_checkbox_reset_default` — **Confirmed 2026-08-08, reported directly:** *"Submit button doesnt reset Checkboxes."* This is a bug in the practice-form fixture itself (`car_insurance_entry/car_insurance_form_wx.py`), not the agent.
  ...
  Fixed: `_check()` now also records each checkbox's declared default in a new `self._checkbox_defaults` dict at construction time; `_clear_all_fields()` resets each checkbox to its own recorded default instead of a hardcoded `False`. Generalizes automatically — no hardcoded list of "the three/six checkboxes that need this," any future checkbox with `default=True` is covered the same way. Text fields and Choice dropdowns are unaffected (Choice already resets to its blank first option by design, a separate and intentional mechanic). 7 new tests (`tests/test_checkbox_reset_to_default.py`), using a real headless `wx.App`/`CarInsuranceFrame` instance rather than mocking `wx` — this is form-widget behavior, nothing meaningful to fake. Full suite: 257 passed, 9 skipped, same 2 pre-existing unrelated tesseract failures. Not yet re-verified live (would show up as a run's second-record-onward fields needing fewer re-check steps on these six).
