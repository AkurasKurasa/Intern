...
- [ ] `execution_no_prompt_caching` — No prompt caching: each step is a fresh LLM call, no batching/budget. **Investigated 2026-08-07** while diagnosing why live runs felt slow (measured: LLM round-trips 3-8s each, dominating per-step time at 42.9% LLM dependency in one run). Found this isn't just "not implemented yet" — `_call_openai_compat()` deliberately injects a random `[sid:{uuid}]` tag into the system prompt on every call specifically to defeat LM Studio's cache ("breaks server-side KV-cache accumulation" per the existing comment). System prompt measured at 3,006 chars (~751 tokens), identical every call within a run — that's ~750+ tokens of pure prompt reprocessing wasted per LLM call, unnecessarily. **Not removed** — the comment implies it was fixing a real bug (stale/bled-together cached context across calls), and removing it needs careful testing (verify JSON responses stay correctly isolated per-call) before trusting it, not a blind rip-out. Real next step when picked back up.
  Also fixed the same day, lower-risk: `max_tokens` was 2048 for a short field-value JSON decision (no batching concern, just an oversized, unused ceiling) — capped at 512, paired with tightening the reasoning instruction to "1-2 short sentences" so the cap can't truncate the `<think>` block before the JSON line ever gets emitted. Regression test: `tests/test_llm_call_speed_params.py`.
