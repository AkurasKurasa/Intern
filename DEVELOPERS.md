...
- [x] `execution_scroll_reveals_only_one_field` — **Re-verified live SAME night: the fixes above genuinely reduced the looping, but the user's next report was that each scroll still only "revealed one damn input field" — not the "optimal view" (as many actionable targets visible at once as possible) `navigation_protocol.py` was originally built to maximize.** Root cause: `_scroll_form_down` always did a fixed, small `pyautogui.scroll(-5)` regardless of how much unfilled content remained below the viewport — one small nudge, stop as soon as anything new appeared, repeat.
  ...
  Explicitly told by the user that the mouse-wheel scroll mechanism isn't required — any means of reaching a better view is fine. A precise pixel-distance calculation (jump exactly to the position that fits the most targets) isn't safely calibratable from here, though — scroll-wheel-unit-to-pixel ratio is OS/app-specific and only the user can live-test it. Took the data-driven middle path instead: count how many still-unfilled fillable fields (editcontrol/comboboxcontrol/checkboxcontrol, non-background, empty value) are currently below the viewport, and scale the scroll magnitude by that count — 1 or fewer remaining → small nudge (`-5`, unchanged, avoids overshooting near the end of a tab's content); a few (2-4) → `-15`; many (5+) → `-25`. Generic (no field/task-specific logic, just type + value + position), same discipline as every other fix tonight. Regression tests: `tests/test_adaptive_scroll_distance.py` (7 new tests). Full suite 323 passed / 9 skipped / same 2 pre-existing unrelated tesseract failures. Not yet re-verified live — the exact thresholds (1/4/5) and magnitudes (-5/-15/-25) are a reasonable starting guess, not something provable without a live run, and may need tuning based on what the user actually sees next.
