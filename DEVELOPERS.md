...
- [ ] `learning_state_encoder_split_projection` — **2026-08-07, queued as the next lever, now against the restored 68.9% checkpoint (5 sessions) after the data-volume retrain regressed to 57.8% — see the correction in `learning_val_click_acc_inflated` above.** The confusion pattern in both checkpoints' per-tab breakdowns is overwhelmingly same-neighborhood, similar-label mixups ("DL Expiration"→"DL Issuing State", "Comprehensive Deductible"→"Collision Deductible") — text embeddings for these pairs are close by construction (they share words), while their bbox positions are what actually distinguishes them. `StateEncoder` fed the full 395-dim per-element vector (11 structured features + 384-dim text embedding) through one shared `nn.Linear`. With the embedding outnumbering structured features 35-to-1, gradient descent has a much easier time finding shortcuts through the huge embedding block than learning to use the 4 bbox dims for fine positional disambiguation — a structural bias toward "right neighborhood, wrong exact field" independent of data volume. Fixed: `StateEncoder` now projects structured features and the text embedding through separate linear layers (each gets a fixed share of `d_model`) before combining, so position/type/filled signal can't be diluted by the embedding's sheer size. General architecture change (any UI has this same position-vs-label-text tradeoff), not task-specific. Regression tests: `tests/test_state_encoder_split_projection.py` (shape/mask correctness, structured-only and embedding-only perturbations both change the output, gradients reach both projections). Not yet trained — should run on the clean 5-session/2,947-sample dataset (the 68.9% baseline) before revisiting data volume, keeping one variable isolated at a time.
