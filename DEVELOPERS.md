...
- [x] **Driver 2/3 fields overwritten with Driver 1's values [FOUND + FIXED 2026-07-13]** —
  NOT the multi-record contamination bug (this is within ONE submission, across sections).
  ROOT CAUSE, confirmed via the run's own scroll warnings ('no scrollable pane found',
  'panel DID NOT MOVE' right after switching to Drivers): `_view_mismatches` walked EVERY
  element UIA reports, including OFF-SCREEN ones. A verify pass that never scrolled past the
  tab's top left Driver 2/3's elements in the list with STALE bbox coords overlapping Driver
  1's area — `_detect_section`'s geometry (lowest section pane above the field's Y-center)
  picked the WRONG section, and `_lookup_field`'s bare-key fallback did the rest. FIX:
  restrict `_view_mismatches` to on-screen elements (same viewport bound every fill path
