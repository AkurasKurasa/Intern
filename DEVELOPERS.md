...
- [ ] `execution_verify_at_fill` — **Gap identified 2026-08-07, from a direct user report about the old (deleted) Intern iteration**: "it lacked a verify-at-fill... we don't want to keep coming back to something, we want it finished as the Agent executes or fills it, a constant checkback always consumes too much time." Confirmed the gap exists in the CURRENT codebase too: `StateValidator.validate()` (`components/agent/state_validator/state_validator.py`) only checks whether *something* changed after an action (focus moved for a click, value changed at all for a keyboard action) — it never compares the resulting value against what the value was actually supposed to be (the record's expected value, already available via `_lookup_field()`). A field can get typed into successfully (value changes, validator says `ok`) while still being *wrong*, and nothing catches that until — currently — never, since there's no separate re-verification pass either. Fix should be inline, not deferred: right after a type/fill action, re-check the field's value against the expected value immediately, and correct it then (bounded retries) rather than either silently moving on or requiring a costly separate "come back and check everything" pass. Not yet implemented.
