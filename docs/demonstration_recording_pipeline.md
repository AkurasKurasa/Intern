...
1. **The OCR-fallback perception path is currently broken.** `recorder.py`'s last-resort branch (used only if Vision/Excel/UIA are all unavailable) imports `TraceTranslator` and calls `_state_from_pil(...)`. Running that import directly:
   ...
   ```
   `trace_translator/__init__.py` imports classes (`HTMLDetector`, `CVDetector`, `UIElementExtractor`) that the actual `trace_translator.py` doesn't define, and the real `TraceTranslator` class has no `_state_from_pil` method and no `use_cv`/`use_html` constructor params. Masked in practice because UIA is available on nearly every real Windows session — but any recording that genuinely needed the OCR fallback would crash `stop()`. **Still open** — the real detector classes don't exist anywhere in the package (`pytest.ini` already documents this independently), so fixing this for real means building HTML/CV detection from scratch, not patching an import.

2. **~~`clean_demos.py` silently breaks on current-format recordings.~~ FIXED 2026-08-19.** It resolved what a click landed on entirely via `t.get("next_state", {})`. As noted in §4, sessions from ~May 2026 onward have no `next_state` field at all (removed as redundant). Run against those, `next_state` defaulted to `{}`, every click resolved to no element, and every mouse-driven step got classified as junk and dropped — only keyboard-only steps survived. Fixed to fall back to the next file's own `state` when the literal field is absent. See `docs/clean_demos_next_state_fix.md` for the full writeup and `tests/test_clean_demos_next_state.py` for regression coverage.
