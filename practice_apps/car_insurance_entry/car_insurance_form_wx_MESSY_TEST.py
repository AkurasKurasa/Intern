...
# "Disgruntled UI" test variant -- deliberately jarring/clashing theme so the
# form LOOKS nothing like the real one, while every field's internal name
# and UIA label wiring below is completely untouched. This file is a COPY,
# not an edit of the real car_insurance_form_wx.py -- the original app used
# by every existing recording/test is unaffected.
ACCENT     = wx.Colour(214, 40, 40)    # clashing red header/section bars
ACCENT_LT  = wx.Colour(255, 247, 179)  # jarring pale-yellow background
WHITE      = wx.Colour(255, 255, 255)
SECT_FG    = wx.Colour(0, 0, 0)        # black text on red bars -- ugly on purpose
HDR_BG     = wx.Colour(30,  200,  90)  # clashing green header
HDR_FG     = wx.Colour(255, 0, 200)    # magenta header text
BORDER_CLR = wx.Colour(20, 20, 20)
...
class CarInsuranceFrame(wx.Frame):
    ...
    def __init__(self, parent):
        super().__init__(parent,
                         title="Car Insurance — Data Entry Form (Messy UI Test)",
                         size=(1040, 820),
    ...
    def _build_ui(self):
        ...
        title = wx.StaticText(hdr, label="  Car Insurance — Data Entry Form (Messy UI Test)",
                              name="form_title")
    ...
    def _build_policy_tab(self):
        ...
        # Deliberately shuffled row order vs. the real form -- same internal
        # control names (policy_number, policy_status, etc.) and same UIA
        # label wiring via _row(), just a different on-screen sequence. Tests
        # whether the agent finds fields by label/meaning or by memorized
        # position.
