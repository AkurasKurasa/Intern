...
def test_modules_that_exist_sit_where_the_layout_says():
    for relative in [
        "coworker_recorder/excel_recorder.py",
        "coworker_recorder/extension/manifest.json",
        "coworker_recorder/extension/content.js",
        "coworker_recorder/extension/background.js",
        "coworker_recorder/reconciler.py",
        "labeling/resolve.py",
        ...
        "data/sheets",
        "data/runs",
        # "tests" intentionally omitted here: this project keeps ALL tests
        # under one shared top-level tests/ root (matching every other
        # component's own convention, not just this one), so the ported
        # suite lives at tests/scope2/ instead of nested inside
        # components/scope2/ -- a deliberate structural adaptation, not a
        # missing file. Checked directly: tests/scope2/ has 9 files, 172+
        # tests, this very file included.
    ]:
        assert (REPO / relative).exists(), f"5 lists {relative}, it is missing"
    # "mocksite" also checked separately, not in the loop above: co-located
    # with the OTHER scope's practice-fixture app (practice_apps/
    # car_insurance_entry/) under a shared practice_apps/ root, at direct
    # request for a clean, consistent structure -- rather than nested
    # inside components/scope2/ the way the rest of this file's own layout
    # lists it. Deliberate, not missing.
    PRACTICE_APPS = REPO.parents[1] / "practice_apps"
    assert (PRACTICE_APPS / "mocksite").exists(), "mocksite is missing from practice_apps/"
...
def test_additions_to_the_layout_are_intentional():
    ...
    additions = {
        ...
        "coworker_recorder/events.py",
        # Milestone 4's reproducible demonstration harness.
        "coworker_recorder/demo_session.py",
