...
class TestHandleEntryAutoDraft:
    ...
    def test_route_scope1_boundary_unaffected_by_reply_agent_presence(self, tmp_path):
        # Sanity: wiring in reply_agent/gmail_client must not touch the
        # existing, separate, non-negotiable Scope #1 safety boundary.
        registry = _build_registry(tmp_path)
        popen = FakePopen()
        ...
        entry = _entry("route_scope1", capsule_name="form_filling", message_id="m1")
