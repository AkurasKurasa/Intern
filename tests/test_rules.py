"""Milestone 6: Remarks is correctly derived from Grade on V0, on both scales.

The direction test is the one that matters. A rule that is right on 0-100 and
silently backwards on 1.00-5.00 would mislabel every student on half the
instrument, and 3.8 is explicit that the operator must be induced.
...
needs_sessions = pytest.mark.skipif(
    ...
    reason="run recorder/demo_session.py for v0 and v6b_scale first",
)
