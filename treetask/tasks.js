        ...
        },
        {
          ...
          children: [
            ...
            { id: "dead-widgets", title: "Stubborn cases: long dropdowns + paste-reject spins", status: "partial",
              note: "MECHANICS probe-verified 2026-07-11 (scratch/probe_dead_widgets.py drives the real _act_on_element): PASS spin ''→'3' (UIA ValuePattern; RangeValue added), PASS 'State' ''→'Texas' (below-fold item; winner = FIRST-LETTER CYCLING with read-back — wx.Choice matches single chars, typing 'Texas' landed on 'South Carolina'). LIVE RUN then exposed the gap the user predicted: the CLICK-FILL combobox path (and reveal-focus path) had no rescue wired — 'Texas' failed 2× and skipped. All THREE combobox paths now route through the pipeline (drill PASSED 22:44: both 50-state widgets ~1s each); spin rescue LIVE-VERIFIED 22:49 ('Years at Address' filled via UIA write). RESIDUAL found same run: the written spin value is INVISIBLE to the observer (state value stays '') → model re-targets the filled field ~15 steps → wrongly dead-marked. DO: read back via the same _resolve_live_control identity the writer used, or mask rescue-filled keys as attempted+filled regardless of observed value." },
          ],
        },
        {
          id: "model-drives",
          ...
          children: [
            ...
            { id: "verb-loop", title: "Agent acts on the model's intents directly", status: "pending",
              ...
              note: "predict() already outputs verbs; the agent still converts to legacy click/type dicts. Wiring verbs → identity executor is what collapses LLM dependency and retires the remaining navigation crutches.\n\nLLM-45% ATTACK PLAN:\n1. [IMPLEMENTED 2026-07-11, untested live] Deterministic value short-circuit — _expected already resolved (section-aware lookup+refresh+bounded peek) now TYPES DIRECTLY, no echo call; LLM only when the record can't resolve the label. Steps tagged 'deterministic' (new metrics line) so LLM Dependency measures real calls. Expect 45%→~15%.\n2. Deterministic next-tab — first unvisited tab; stop phoning the LLM for it.\n3. Verb-loop (this node) — verbs+pointer straight into the identity executor; makes <5% structural. Sweep inherits the resolver.\nMeasure: 'LLM Dependency' + 'Deterministic Steps' lines per run." },
          ],
        },
        {
          id: "good-data",
