  ...
  children: [
      ...
      children: [
        ...
        },
        {
          ...
          children: [
            ...
            { id: "driver-section-crosstalk", owner: "Ralph", title: "Driver 2/3 don't get overwritten with Driver 1's values", status: "done",
              note: "FOUND + FIXED 2026-07-13, full acceptance run. ROOT CAUSE (confirmed via the run's own scroll warnings — 'no scrollable pane found', 'panel DID NOT MOVE' — right after switching to Drivers): _view_mismatches walked EVERY element UIA reports, including OFF-SCREEN ones. When a verify pass never scrolled past the tab's top, Driver 2/3's elements stayed in the list with STALE bbox coords overlapping Driver 1's area — _detect_section's geometry math (lowest section pane above the field's Y-center) picked the WRONG section, and _lookup_field's bare-key fallback did the rest: Driver 2's fields got silently overwritten with Driver 1's values. FIX: restrict _view_mismatches to on-screen elements (same viewport bound every fill path already uses) — an off-screen field's mismatch gets caught on a LATER pass once actually visible, so nothing is skipped, just deferred. Offline-verified: reproduced the exact failure (off-screen Driver-2 field, stale bbox) → now correctly excluded; on-screen wrong Driver-2 field → still correctly caught with the section-correct expected value. LIVE-CONFIRMED 2026-07-13 rerun: zero Driver 2/3 cross-contamination (D7734821/F8821047-Maria-Delgado/D0012938 all correct), BC top-mismatches empty, verify pass clean on the final scan.",
            },
    ...
    },
    {
      ...
      children: [
        ...
        { id: "behavioral-match", title: "Sequence-fidelity metric actually measures", status: "done",
          note: "FIXED 2026-07-12: reference wired to the training corpus. FOUND STILL BROKEN 2026-07-13 (dead empty dir shadowed the real corpus via plain .exists()) → fixed same day (check for session_* subfolders). LIVE-CONFIRMED 2026-07-13 rerun: Behavioral Match printed a REAL number — 13.5% (seq_sim 13.5%, tab_order 0%, agent=90 actions vs ref=250). CALIBRATION: demo-vs-demo similarity = 81% is the human ceiling — 13.5% is real signal, low because the agent's single-record run (90 actions) is far shorter than the multi-tab reference session (250 actions) and tab-visit order isn't yet optimized for sequence match. The metric MOVES now — that was the fix's whole job. Improving the number itself is a separate, later task (train on re-cleaned demos / align tab order)." },
      ],
    },
  ],
};
