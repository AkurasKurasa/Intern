  ...
  children: [
    {
      ...
      children: [
        ...
        },
        {
          ...
          status: "partial",
          priority: 2,
          note: "Run scorer (eval_metrics) FIXED 2026-07-10 (41cff4c): section-aware value matching. bc_fidelity CONFIRMED BROKEN by the ×2 probe (2026-07-11) — two subtasks below.",
          children: [
            { id: "bc-record-gold", title: "bc_fidelity grades each record vs ITS OWN answers", status: "next",
              note: "×2 probe: record 2 graded against record 1's gold (expected 'PAI-2026-00441', got '...442' — the grader holds the wrong answer sheet; record 2's BC 15.5% is fiction). DO: pick gold per record — the submission JSON filename carries the policy number." },
            { id: "bc-section-gold", title: "bc_fidelity gold keys are section-aware", status: "pending",
              note: "Record 1 block claims ph_first expected 'Tyler' when the policyholder IS James (Tyler = a Driver section) — gold keys mapped to the wrong section. DO: same section-first repair eval_metrics got (41cff4c)." },
            { id: "eval-record-gold", title: "Run scorer knows which record it's grading", status: "pending",
              note: "Found in the --start_record 2 probe: evaluate_run compares typed values with NO record number — record 2's correct 'Underwriter' scored ✗ against record 1's value, while record-1 CONTAMINATION scored ✓. A scorer that rewards the bug and punishes the fix. DO: pass record_num into evaluate_run, bound its source parse with _record_line_span / _parse_records." },
          ],
        },
        {
          id: "sections-work",
        ...
        },
        {
          ...
          children: [
            ...
            { id: "identity-everywhere", title: "ALL fill paths use it (then delete the pixel-math guards)",
              ...
              note: "OPT2 fill + verify-fixes still use click+paste. Routing them through the identity executor retires the stale-coordinate bug class for good. Includes the chronic dead widgets: SpinCtrls ('Years Continuously Insured', 'Accidents (3 yr)') and fold-edge 50-state combos burn retries + dead-mark EVERY run — RangeValue/SelectionItem patterns fill them directly." },
        ...
        },
        {
          ...
          children: [
            ...
            { id: "verb-loop", title: "Agent acts on the model's intents directly", status: "pending",
              ...
              note: "predict() already outputs verbs; the agent still converts to legacy click/type dicts. Wiring verbs → identity executor is what collapses LLM dependency and retires the remaining navigation crutches.\n\nLLM-45% ATTACK PLAN (2026-07-11, ordered by payoff/cost):\n1. Deterministic value lookup FIRST — exact/normalized label+section match against the cached record; LLM only for ambiguous labels ('DOB' vs 'Date of Birth'). ~90% of labels match exactly → biggest single cut (45%→~15%), one day, no retrain.\n2. Deterministic next-tab — first unvisited tab; stop phoning the LLM for it.\n3. Verb-loop (this node) — verbs+pointer straight into the identity executor; makes <5% structural. Sweep inherits resolver from step 1.\nMeasure: 'LLM Dependency' line in run metrics after each step." },
          ],
        },
        {
          id: "good-data",
